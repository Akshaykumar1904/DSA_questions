...
//        System.out.println(fixedPoint(new int[]{0,2,5,8,7}));
...
//        int element = nums[index];
    ...
    */
    ...
    public static int findPeakElement(int[] nums) {
        ...
        while (start < end) {
            ...
            if (nums[mid] > nums[mid + 1]) {
                // you're in the decreasing part → peak is to the left or at mid
                end = mid;
            } else {
    ...
    public static int findMin(int[] nums) {
        if(findPivotIndex(nums)==-1){
        return nums[0];
        }
        return nums[findPivotIndex(nums)+1];
    }
    ...
    public static int minEatingSpeed(int[] weights, int h) {
        int low = 1;
        int high = highest(weights);
        int ans = high;
        while(low<=high){
            int middle = low +(high-low)/2;
            if(isValid(weights,h,middle)){
                ans = middle;
                high = middle -1;
            }else{
                low = middle +1;
    ...
    private static boolean isValid(int[] arr, int h,int value) {
        long sum=0;
        for(int num:arr){
            sum += (long)Math.ceil((double) num /value);
        }
        return sum <= h;
    }
